...
function verifyServer(){
   ...
   function verify(data, i){
         if (data.status==('offline')){
            document.getElementById(idsLabels[i]).innerHTML='El servidor '+data.host+ ' a las '+data.time+' no esta en funcionamiento'
            showButton(idsButtons[i])
            changeIndicator('r', idsIndicators[i])
        }  else if(data.code==('200')){
            document.getElementById(idsLabels[i]).innerHTML='El servidor '+data.host+ ' a las '+data.time+' esta en funcionamiento'                    
            changeIndicator('g', idsIndicators[i])
        }   else {
            document.getElementById(idsLabels[i]).innerHTML='El servidor '+data.host+ ' a las '+data.time+' esta en funcionamiento pero es un caso especial que no recuerdo jajaj XD'                    
